...
OUT_ROOT="${OUT_ROOT:-$ROOT/results/baselines/p5_style_envmap_strict_eval3_${TIMESTAMP_TAG}}"
...
echo "[$(date '+%F %T')] p5_style_envmap_strict_eval3_start" | tee -a "$OUT_ROOT/suite.log"
...
echo "claim_boundary=P5-style constrained item generator, not official OpenP5" | tee -a "$OUT_ROOT/suite.log"

if [ ! -f "$P5_DIR/best.pt" ]; then
  mkdir -p "$P5_DIR"
  echo "[$(date '+%F %T')] train P5-style constrained generator with first_seen/env item mapping" | tee -a "$OUT_ROOT/suite.log"
  ...
  echo "[$(date '+%F %T')] reuse P5-style checkpoint $P5_DIR/best.pt" | tee -a "$OUT_ROOT/suite.log"
fi

for eval_seed in "${EVAL_SEEDS[@]}"; do
  log_path="$OUT_ROOT/eval_P5Style_seed${eval_seed}.log"
  echo "[$(date '+%F %T')] eval P5-style seed=$eval_seed" | tee -a "$OUT_ROOT/suite.log"
  "$PYTHON_BIN" code/eval_p5_style_env.py \
    --p5_style_ckpt "$P5_DIR/best.pt" \
...
echo "[$(date '+%F %T')] p5_style_envmap_strict_eval3_done" | tee -a "$OUT_ROOT/suite.log"
