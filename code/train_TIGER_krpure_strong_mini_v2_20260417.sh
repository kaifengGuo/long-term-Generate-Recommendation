...
SAVE_PATH="${SAVE_PATH:-${ROOT_PATH}/output/KuaiRand_Pure/env/tiger_sid_krpure_mini_engagev2_seed2026_${TIMESTAMP_TAG}.pth}"
TRAIN_LOG_PATH="${TRAIN_LOG_PATH:-${ROOT_PATH}/output/KuaiRand_Pure/env/log/tiger_sid_krpure_mini_engagev2_seed2026_${TIMESTAMP_TAG}.log}"
EVAL_LOG_PATH="${EVAL_LOG_PATH:-${ROOT_PATH}/output/KuaiRand_Pure/env/log/tiger_sid_krpure_mini_engagev2_seed2026_eval2026_${TIMESTAMP_TAG}.log}"

mkdir -p "$(dirname "${SAVE_PATH}")" "$(dirname "${TRAIN_LOG_PATH}")" "$(dirname "${EVAL_LOG_PATH}")"

echo "[train-v2] save_path=${SAVE_PATH}"
echo "[train-v2] train_log=${TRAIN_LOG_PATH}"
echo "[train-v2] eval_log=${EVAL_LOG_PATH}"
...
"${PYTHON_BIN}" "${ROOT_PATH}/code/train_TIGER_krpure.py" \
  ...
  --warmup_ratio 0.06 \
  --min_lr_ratio 0.10 \
  --target_behavior_cols "is_click,long_view,is_like,is_comment,is_follow,is_forward" \
  --history_behavior_cols "is_click,long_view,is_like,is_comment,is_follow,is_forward" \
  --behavior_weight_spec "is_click:1.0,long_view:0.75,is_like:1.25,is_comment:1.5,is_follow:2.0,is_forward:1.25" \
  --positive_threshold 0.0 \
  --sample_weight_mode log1p \
