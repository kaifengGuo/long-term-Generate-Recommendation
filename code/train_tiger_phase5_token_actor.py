...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train a token-aware residual actor head for TIGER decode-time nudging.")
    ...
    parser.add_argument("--support_scale", type=float, default=0.50)
    parser.add_argument("--neg_scale", type=float, default=0.30)
    parser.add_argument("--anchor_scale", type=float, default=0.05)
    parser.add_argument("--credit_clip", type=float, default=3.0)
    parser.add_argument("--residual_clip", type=float, default=3.0)
    parser.add_argument("--save_dir", type=str, default="")
    parser.add_argument("--metrics_out", type=str, default="")
    return parser.parse_args()


class Phase5TokenActorDataset(Dataset):
    ...
    def __getitem__(self, idx: int):
        ...
        return (
            ...
            torch.tensor(float(row["support_strength"]), dtype=torch.float32),
            row["group"],
        )
...
def collate_phase5(batch):
    ...
    support_strength = torch.stack([x[5] for x in batch], dim=0)
    groups = [x[6] for x in batch]
    return input_ids, attention_mask, target_tokens, token_credit, item_credit, support_strength, groups
    ...
    with chain_path.open("r", encoding="utf-8") as f:
        for line in f:
            ...
            if len(target_tokens) != sid_depth or len(token_credit) != sid_depth:
                continue
            if max(abs(float(item_credit)), max((abs(float(x)) for x in token_credit), default=0.0)) < float(min_abs_credit):
                continue
...
def compute_phase5_loss(
    ...
    *,
    ...
    support_mult = 1.0 + float(support_scale) * support_strength.unsqueeze(-1)
    pos_w = support_mult * (torch.relu(token_credit) + float(item_weight_scale) * torch.relu(item_credit).unsqueeze(-1))
    neg_w = support_mult * torch.relu(-token_credit)
...
def evaluate_actor(tiger, actor_head, loader: DataLoader, device: torch.device, args) -> Dict[str, float]:
    ...
    probs: List[float] = []
    with torch.no_grad():
        for input_ids, attention_mask, target_tokens, token_credit, item_credit, support_strength, _groups in loader:
            input_ids = input_ids.to(device)
            attention_mask = attention_mask.to(device)
            target_tokens = target_tokens.to(device)
            token_credit = token_credit.to(device)
            item_credit = item_credit.to(device)
            support_strength = support_strength.to(device)
            base_logits, hidden = tiger.decode_with_hidden(
                input_ids=input_ids,
                attention_mask=attention_mask,
                decoder_input_ids=decoder_input_ids_from_targets(target_tokens),
            )
            residual_scores = score_all_residuals(actor_head, hidden.detach())
            loss, stats = compute_phase5_loss(
                base_logits,
                residual_scores,
                target_tokens,
                token_credit,
            ...
            gains.append(float(stats["target_prob"] - stats["base_target_prob"]))
    return {
        "actor_loss": float(np.mean(losses)) if losses else 0.0,
        "target_prob": float(np.mean(probs)) if probs else 0.0,
        "target_gain": float(np.mean(gains)) if gains else 0.0,
    }
...
def main() -> int:
    ...
    dataset = Phase5TokenActorDataset(chain_rows)
    groups = [row["group"] for row in chain_rows]
    train_idx, valid_idx = split_groups(groups, float(args.valid_ratio), int(args.seed))
    train_loader = DataLoader(Subset(dataset, train_idx.tolist()), batch_size=int(args.batch_size), shuffle=True, num_workers=0, collate_fn=collate_phase5)
    valid_loader = DataLoader(Subset(dataset, valid_idx.tolist()), batch_size=int(args.batch_size), shuffle=False, num_workers=0, collate_fn=collate_phase5)

    actor_head = TokenResidualActorHead(
    ...
    for epoch in range(1, int(args.epochs) + 1):
        ...
        for input_ids, attention_mask, target_tokens, token_credit, item_credit, support_strength, _groups in train_loader:
            input_ids = input_ids.to(device)
            attention_mask = attention_mask.to(device)
            target_tokens = target_tokens.to(device)
            token_credit = token_credit.to(device)
            ...
            train_gains.append(float(stats["target_prob"] - stats["base_target_prob"]))
        ...
        print(
            ...
            f"train_target_gain={valid_metrics['train_target_gain']:.4f} "
    ...
    save_dir = Path(args.save_dir) if args.save_dir else Path(args.tiger_ckpt).resolve().parent / "phase5_token_actor"
    save_dir.mkdir(parents=True, exist_ok=True)
    head_path = save_dir / "phase5_token_actor_head.pt"
    meta_path = save_dir / "phase5_token_actor_meta.json"
    metrics_path = Path(args.metrics_out) if args.metrics_out else save_dir / "phase5_token_actor_metrics.json"

    torch.save({"model_state_dict": best_state}, head_path)
    meta = {
        "method": "TIGER Phase5 Token Residual Actor",
        ...
        "support_field": str(args.support_field),
        "item_weight_scale": float(args.item_weight_scale),
        "support_scale": float(args.support_scale),
        "neg_scale": float(args.neg_scale),
        "anchor_scale": float(args.anchor_scale),
    ...
    print(f"[phase5] saved actor head to {head_path}")
    print(f"[phase5] saved actor meta to {meta_path}")
    return 0
