...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Phase8c sparse raw hierarchical actor fine-tune for TIGER."
    )
    ...
    parser.add_argument("--token_credit_field", type=str, default="token_adv_prefix_diff")
    parser.add_argument("--item_credit_field", type=str, default="item_adv_prefix_diff")
    parser.add_argument("--page_credit_field", type=str, default="page_adv_prefix_diff")
    parser.add_argument("--min_abs_credit", type=float, default=1e-6)
    ...
    parser.add_argument("--item_weight_scale", type=float, default=0.10)
    parser.add_argument("--item_neg_scale", type=float, default=0.05)
    parser.add_argument("--neg_scale", type=float, default=0.10)
    parser.add_argument("--kl_scale", type=float, default=0.25)
    ...
    parser.add_argument("--page_gate_scale", type=float, default=0.20)
    parser.add_argument("--page_gate_min", type=float, default=0.80)
    parser.add_argument("--page_gate_max", type=float, default=1.20)
...
class Phase8cActorDataset(Dataset):
...
def collate_rows(batch: Sequence[Dict[str, Any]]) -> Dict[str, Any]:
    return {
        ...
        "token_credit": torch.stack([torch.tensor(x["token_credit"], dtype=torch.float32) for x in batch], dim=0),
        "item_credit": torch.tensor([float(x["item_credit"]) for x in batch], dtype=torch.float32),
        "page_credit": torch.tensor([float(x["page_credit"]) for x in batch], dtype=torch.float32),
...
def load_chain_rows(
    ...
    page_credit_field: str,
    min_abs_credit: float,
    max_rows: int,
    ...
    with chain_path.open("r", encoding="utf-8") as f:
        for line_idx, line in enumerate(f):
            ...
            token_credit = [float(x) for x in payload.get(str(token_credit_field), [])]
            item_credit = float(payload.get(str(item_credit_field), 0.0))
            page_credit = float(payload.get(str(page_credit_field), 0.0))
            if len(target_tokens) != sid_depth or len(token_credit) != sid_depth:
                continue
            if (
                max(
                    abs(float(item_credit)),
                    abs(float(page_credit)),
                    max((abs(float(x)) for x in token_credit), default=0.0),
                )
                < float(min_abs_credit)
            ):
                continue
...
def build_page_gate(
    ...
    *,
    ...
    signal = renorm_signal(page_credit, str(renorm_mode))
    gate = 1.0 + float(page_gate_scale) * torch.tanh(signal)
    gate = gate.clamp(min=float(page_gate_min), max=float(page_gate_max))
...
def compute_actor_loss(
    ...
    *,
    ...
    if float(credit_clip) > 0.0:
        ...
        page_credit = page_credit.clamp(min=-float(credit_clip), max=float(credit_clip))

    token_credit = renorm_signal(token_credit, str(renorm_mode))
    item_credit = renorm_signal(item_credit, str(renorm_mode))
    page_gate = build_page_gate(
        page_credit,
    ...
    pos_score = torch.relu(token_credit)
    neg_score = torch.relu(-token_credit)
    pos_score = page_gate * pos_score * (1.0 + float(item_weight_scale) * torch.relu(item_credit).unsqueeze(-1))
    neg_score = page_gate * neg_score * (1.0 + float(item_neg_scale) * torch.relu(-item_credit).unsqueeze(-1))

    pos_mask = build_sparse_mask(pos_score, int(positive_topk), float(positive_floor))
    neg_mask = build_sparse_mask(neg_score, int(negative_topk), float(negative_floor))
    pos_w = pos_score * pos_mask
    neg_w = neg_score * neg_mask

    pos_loss = (pos_w * ce).sum() / (pos_w.sum() + 1e-8)
    neg_excess = torch.relu(actor_target_prob - base_target_prob)
    neg_loss = (neg_w * neg_excess).sum() / (neg_w.sum() + 1e-8)
    base_probs = base_log_probs.exp()
    kl_loss = F.kl_div(actor_log_probs, base_probs, reduction="batchmean", log_target=False)
    loss = pos_loss + float(neg_scale) * neg_loss + float(kl_scale) * kl_loss

    stats = {
        "pos_loss": float(pos_loss.item()),
        "neg_loss": float(neg_loss.item()),
        "kl_loss": float(kl_loss.item()),
        "target_prob": float(actor_target_prob.mean().item()),
        "base_target_prob": float(base_target_prob.mean().item()),
        "target_gain": float((actor_target_prob - base_target_prob).mean().item()),
        "pos_weight": float(pos_w.mean().item()),
        "neg_weight": float(neg_w.mean().item()),
        "page_gate_mean": float(page_gate.mean().item()),
        "pos_active_frac": float((pos_w > 0).float().mean().item()),
        "neg_active_frac": float((neg_w > 0).float().mean().item()),
        "pos_selected_per_row": float((pos_w > 0).float().sum(dim=-1).mean().item()),
        "neg_selected_per_row": float((neg_w > 0).float().sum(dim=-1).mean().item()),
    }
    return loss, stats
...
def forward_actor(
    ...
    page_credit = batch["page_credit"].to(device)

    decoder_input_ids = decoder_input_ids_from_targets(target_tokens)
    with torch.no_grad():
        base_logits, _ = base_tiger.decode_with_hidden(
            input_ids=input_ids,
    ...
    loss, stats = compute_actor_loss(
        ...
        item_neg_scale=float(args.item_neg_scale),
...
@torch.no_grad()
def evaluate_actor(actor_tiger, base_tiger, loader: DataLoader, device: torch.device, args) -> Dict[str, float]:
    actor_tiger.eval()
    metrics: Dict[str, List[float]] = {}
    for batch in loader:
        _loss, stats = forward_actor(actor_tiger, base_tiger, batch, device, args)
...
def main() -> int:
    ...
    chain_rows, sid_depth, vocab_size = load_chain_rows(
        ...
        str(args.page_credit_field),
        float(args.min_abs_credit),
        int(args.max_rows),
    )

    size_cfg = infer_model_size_args(str(args.model_size))
    base_tiger, _sid_depth_model, _codebook_size = load_tiger_model(
    ...
    dataset = Phase8cActorDataset(chain_rows)
    ...
    for epoch in range(1, int(args.epochs) + 1):
        ...
        train_gains: List[float] = []
        for batch in train_loader:
            loss, stats = forward_actor(actor_tiger, base_tiger, batch, device, args)
            optimizer.zero_grad(set_to_none=True)
            loss.backward()
            optimizer.step()
            train_losses.append(float(loss.item()))
            train_gains.append(float(stats["target_gain"]))

        valid_metrics = evaluate_actor(actor_tiger, base_tiger, valid_loader, device, args)
        valid_metrics["epoch"] = float(epoch)
        valid_metrics["train_loss"] = float(np.mean(train_losses)) if train_losses else 0.0
        valid_metrics["train_target_gain"] = float(np.mean(train_gains)) if train_gains else 0.0
        ...
        print(
            ...
            f"valid_gain={valid_metrics['target_gain']:.4f} "
            f"pos_frac={valid_metrics['pos_active_frac']:.4f} "
            f"neg_frac={valid_metrics['neg_active_frac']:.4f} "
            f"gate={valid_metrics['page_gate_mean']:.4f} "
            f"kl={valid_metrics['kl_loss']:.4f}"
        )

    if best_state is None:
        raise RuntimeError("Phase8c sparse raw actor fine-tune produced no checkpoint.")

    save_dir = (
        Path(args.save_dir)
        if args.save_dir
        else Path(args.tiger_ckpt).resolve().parent / "phase8c_sparse_raw_actor"
    )
    save_dir.mkdir(parents=True, exist_ok=True)
    ckpt_path = save_dir / "phase8c_sparse_raw_actor_tiger.pth"
    meta_path = save_dir / "phase8c_sparse_raw_actor_meta.json"
    metrics_path = (
        Path(args.metrics_out)
        if args.metrics_out
        else save_dir / "phase8c_sparse_raw_actor_metrics.json"
    )
    torch.save(best_state, ckpt_path)
    meta = {
        "method": "TIGER Phase8c Sparse Raw Hierarchical Actor FineTune",
        ...
        "page_credit_field": str(args.page_credit_field),
    ...
    print(f"[phase8c] saved fine-tuned TIGER to {ckpt_path}")
    print(f"[phase8c] saved meta to {meta_path}")
    return 0
