...
class BaseRLAgent():
    ...
    def parse_model_args(parser):
        ...
        parser.add_argument('--snapshot_episode', type=int, default=0,
                            help='if >0, keep snapshot checkpoints every N iterations')
    ...
    def __init__(self, *input_args):
        ...
        self.snapshot_episode = args.snapshot_episode
    ...
    def train(self):
        ...
        for i in tqdm(range(step_offset, step_offset + self.n_iter[-1]//10)):
            ...
            if i % self.save_episode == 0:
                self.save()
            if self.snapshot_episode > 0 and i > 0 and i % self.snapshot_episode == 0:
                self.save(self._build_snapshot_path(i))
    ...
    def _build_snapshot_path(self, step_idx):
        return f"{self.save_path}_step{int(step_idx):05d}"

    def _resolve_save_path(self, save_path=None):
        return self.save_path if save_path is None else save_path

    def save(self, save_path=None):
        target_path = self._resolve_save_path(save_path)
        for model, opt, prefix in self.registered_models:
            torch.save(model.state_dict(), target_path + prefix)
            torch.save(opt.state_dict(), target_path + prefix + "_optimizer")
    
    def load(self, save_path=None):
        target_path = self._resolve_save_path(save_path)
        for model, opt, prefix in self.registered_models:
            model.load_state_dict(torch.load(target_path + prefix, map_location = self.device))
            opt.load_state_dict(torch.load(target_path + prefix + "_optimizer", map_location = self.device))
