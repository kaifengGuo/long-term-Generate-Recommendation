...
class HAC(DDPG):
    ...
    def parse_model_args(parser):
        ...
        parser.add_argument('--hyper_actor_lr', type=float, default=0.0001,
                            help='optimizer learning rate for inverse/hyper actor branch')
        return parser
    ...
    def __init__(self, *input_args):
        ...
        self.hyper_actor_lr = args.hyper_actor_lr
        ...
        self.inverse_module_optimizer = torch.optim.Adam(chain(self.inverse_module.parameters(), self.actor.parameters()), 
                                                         lr=args.hyper_actor_lr, weight_decay=args.actor_decay)
    ...
    def step_train(self):
        ...
        critic_loss, actor_loss, hyper_actor_loss, behavior_loss, q_mean, next_q_mean = self.get_hac_loss(
    ...
    def get_hac_loss(self, observation, policy_output, user_feedback, 
        ...
        return critic_loss, actor_loss, hyper_actor_loss, behavior_loss, torch.mean(current_Q).item(), torch.mean(next_Q).item()
    ...
    def load(self, save_path=None):
        super().load(save_path=save_path)
