...
OUT_ROOT="${OUT_ROOT:-$ROOT/results/baselines/seqrec_envmap_strict_eval3_${TIMESTAMP_TAG}}"
SEQ_ROOT="${SEQ_ROOT:-$ROOT/results/baselines/seq_ckpts_envmap_20260512}"
...
P5_DIR="$SEQ_ROOT/p5_style_firstseen_seed${SEQ_TRAIN_SEED}"
...
for policy, items in by_policy.items():
    out = {
        ...
        "ckpt": items[0]["ckpt"],
        "mapping_order": items[0]["mapping_order"],
...
fields = ["policy", "train_seed", "ckpt", "mapping_order", "n_eval_seeds", "eval_seeds"] + [
...
train_bert4rec() {
  if [ -f "$BERT4REC_DIR/best.pt" ]; then
    echo "[$(date '+%F %T')] reuse BERT4Rec first_seen checkpoint $BERT4REC_DIR/best.pt" | tee -a "$OUT_ROOT/suite.log"
    return
  fi
...
train_p5_style() {
  if [ -f "$P5_DIR/best.pt" ]; then
    echo "[$(date '+%F %T')] reuse P5-style first_seen checkpoint $P5_DIR/best.pt" | tee -a "$OUT_ROOT/suite.log"
    return
  fi
  mkdir -p "$P5_DIR"
  echo "[$(date '+%F %T')] train P5-style with first_seen/env item mapping" | tee -a "$OUT_ROOT/suite.log"
  "$PYTHON_BIN" code/train_p5_style_baseline.py \
    --csv_path "$CSV_PATH" \
    --save_dir "$P5_DIR" \
    --cache_dir "$ROOT/code/cache_p5_style_firstseen" \
    --mapping_order first_seen \
    --device cuda:0 \
    --max_len 50 \
    --d_model 128 \
...
eval_policy() {
  ...
  for eval_seed in "${EVAL_SEEDS[@]}"; do
    local log_path="$OUT_ROOT/eval_${policy}_seed${eval_seed}.log"
    echo "[$(date '+%F %T')] eval $policy seed=$eval_seed" | tee -a "$OUT_ROOT/suite.log"
    "$PYTHON_BIN" "$cmd" \
      "$ckpt_arg" "$ckpt_dir/best.pt" \
      ...
      --log_every 50 \
      > "$log_path" 2>&1
    printf '%s\t%s\t%s\tbest.pt\tfirst_seen\t%s\n' \
      "$policy" "$SEQ_TRAIN_SEED" "$eval_seed" "$(parse_eval_log "$log_path")" >> "$OUT_ROOT/per_run.tsv"
  done
}

printf 'policy\ttrain_seed\teval_seed\tckpt\tmapping_order\ttotal_reward\tdepth\tavg_reward\tcoverage\tild\tclick_pct\tlong_view_pct\tlike_pct\tcomment_pct\tforward_pct\tfollow_pct\thate_pct\n' > "$OUT_ROOT/per_run.tsv"

echo "[$(date '+%F %T')] seqrec_envmap_strict_eval3_start" | tee -a "$OUT_ROOT/suite.log"
...
echo "claim_boundary=pure BERT4Rec/P5-style model scoring only; first_seen item mapping aligns training ids with the online env reader." | tee -a "$OUT_ROOT/suite.log"

train_bert4rec
eval_policy "BERT4Rec" "code/eval_bert4rec_env.py" "$BERT4REC_DIR" "--bert4rec_ckpt"

train_p5_style
eval_policy "P5-style" "code/eval_p5_style_env.py" "$P5_DIR" "--p5_style_ckpt"

summarize_runs

echo "[$(date '+%F %T')] seqrec_envmap_strict_eval3_done" | tee -a "$OUT_ROOT/suite.log"
