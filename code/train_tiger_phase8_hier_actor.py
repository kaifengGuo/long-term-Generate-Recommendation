...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Hierarchical calibrated-advantage TIGER actor fine-tune.")
    ...
    parser.add_argument("--item_weight_scale", type=float, default=0.25)
    parser.add_argument("--page_weight_scale", type=float, default=0.25)
...
class Phase8ActorDataset(Dataset):
...
def compute_actor_loss(
    ...
    *,
    ...
    page_credit = renorm_signal(page_credit, str(renorm_mode))
...
def main() -> int:
    ...
    dataset = Phase8ActorDataset(chain_rows)
    ...
    if best_state is None:
        raise RuntimeError("Phase8 hier actor fine-tune produced no checkpoint.")

    save_dir = Path(args.save_dir) if args.save_dir else Path(args.tiger_ckpt).resolve().parent / "phase8_hier_actor"
    save_dir.mkdir(parents=True, exist_ok=True)
    ckpt_path = save_dir / "phase8_hier_actor_tiger.pth"
    meta_path = save_dir / "phase8_hier_actor_meta.json"
    metrics_path = Path(args.metrics_out) if args.metrics_out else save_dir / "phase8_hier_actor_metrics.json"
    torch.save(best_state, ckpt_path)
    meta = {
        "method": "TIGER Phase8 Hierarchical Prefix Actor FineTune",
    ...
    print(f"[phase8] saved fine-tuned TIGER to {ckpt_path}")
    print(f"[phase8] saved meta to {meta_path}")
    return 0
