...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Compare multiple runs on rollout-eval reward.")
    parser.add_argument("--runs", type=str, nargs="+", required=True, help="Each item format: label=summary_json_path")
    parser.add_argument("--output_path", type=str, required=True)
    parser.add_argument("--title", type=str, default="Rollout Reward Comparison")
...
def main() -> int:
    ...
    for idx, (label, path) in enumerate(run_specs):
        ...
        y = [safe_float(row.get("rollout_eval", {}).get("total_reward", float("nan"))) for row in rows]
    ...
    ax.set_ylabel("Rollout total reward")
