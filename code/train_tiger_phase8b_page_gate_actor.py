...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Phase8b TIGER actor fine-tune with page-gated item/token advantages.")
    ...
    parser.add_argument("--token_credit_field", type=str, default="token_adv_prefix_diff_calibrated")
    parser.add_argument("--item_credit_field", type=str, default="item_adv_prefix_diff_calibrated")
    ...
    parser.add_argument("--item_weight_scale", type=float, default=0.15)
    parser.add_argument("--neg_scale", type=float, default=0.20)
    parser.add_argument("--kl_scale", type=float, default=0.20)
    ...
    parser.add_argument("--page_gate_scale", type=float, default=0.35)
    parser.add_argument("--page_gate_min", type=float, default=0.60)
    parser.add_argument("--page_gate_max", type=float, default=1.35)
    parser.add_argument("--save_dir", type=str, default="")
    parser.add_argument("--metrics_out", type=str, default="")
    return parser.parse_args()


class Phase8bActorDataset(Dataset):
...
def compute_actor_loss(
    ...
    *,
    ...
    neg_w = page_gate * (
        torch.relu(-token_credit)
        + 0.5 * float(item_weight_scale) * torch.relu(-item_credit).unsqueeze(-1)
    )
...
def main() -> int:
    ...
    dataset = Phase8bActorDataset(chain_rows)
    ...
    if best_state is None:
        raise RuntimeError("Phase8b page-gate actor fine-tune produced no checkpoint.")

    save_dir = Path(args.save_dir) if args.save_dir else Path(args.tiger_ckpt).resolve().parent / "phase8b_page_gate_actor"
    save_dir.mkdir(parents=True, exist_ok=True)
    ckpt_path = save_dir / "phase8b_page_gate_actor_tiger.pth"
    meta_path = save_dir / "phase8b_page_gate_actor_meta.json"
    metrics_path = Path(args.metrics_out) if args.metrics_out else save_dir / "phase8b_page_gate_actor_metrics.json"
    torch.save(best_state, ckpt_path)
    meta = {
        "method": "TIGER Phase8b Page-Gated Hierarchical Actor FineTune",
    ...
    print(f"[phase8b] saved fine-tuned TIGER to {ckpt_path}")
    print(f"[phase8b] saved meta to {meta_path}")
    return 0
